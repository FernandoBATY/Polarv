...
func update_preview() -> void:
	...
	var snapped: Vector2 = IsoGrid.grid_to_world(cell)

	furniture_preview.global_position = snapped
	furniture_preview.z_index = int(snapped.y)
...
func _update_preview_cells(origin: Vector2i, size: Vector2i, is_valid: bool) -> void:
	...
	for cell in cells:
		...
		cell_sprite.position = IsoGrid.grid_to_world(cell) - origin_world
		cell_sprite.z_index = -10
		cell_sprite.modulate = Color(0, 1, 0, 0.35) if is_valid else Color(1, 0, 0, 0.35)
